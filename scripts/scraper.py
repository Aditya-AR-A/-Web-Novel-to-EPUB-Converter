...
def get_chapter_metadata(url):
    ...
    # Better origin_url detection
    from urllib.parse import urlparse
    parsed = urlparse(url)
    dynamic_origin = f"{parsed.scheme}://{parsed.netloc}" if parsed.netloc else origin_url
    ...
    image_url = meta_content("og:image") or (urljoin(dynamic_origin, img_path) if img_path else None)
    ...
    if not read_first_url:
        ...
        if anchor and anchor.get("href"):
            read_first_url = urljoin(dynamic_origin, anchor.get("href"))

    if not read_first_url:
        # Fallback: Is this page already a chapter page?
        # Check for common chapter content markers
        if soup.find("div", class_="m-read") or soup.find("div", id="article") or soup.select_one(".read-content, .chapter-content"):
            read_first_url = url
            print(f"[DEBUG] No 'Read First' link, but page looks like a chapter. Using {url}")
        else:
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for i, pair in enumerate(indexed):
            ctx = contextvars.copy_context()  # fresh copy per task — a single Context cannot be entered concurrently
            future = ex.submit(ctx.run, worker, pair, i)
