...
def to_epub(metadata, chapter_data, chapters_per_book=500, *, output_dir: str | os.PathLike[str] | None = None, start_chapter_offset: int = 0):
    ...
    for book_index in range(num_books):
        ...
        for local_idx in range(local_start, local_end):
            raise_if_cancelled()
            abs_idx = start_chapter_offset + local_idx   # absolute 0-based chapter index
        ...
        filename = f"{slug}-ch-{abs_ch_start}-{abs_ch_end}.epub"
