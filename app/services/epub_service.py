...
class EpubService:
    ...
    def create_epub(
        ...
        *,
        ...
        storage_key = f"epubs/{epub_filename}"
        ...
        try:
            storage_key_result, storage_url, file_size = self._generate_and_upload(
                url=url,
                title=title,
                author=author,
                genres=genres or [],
        ...
        with get_session() as session:
            ...
            db_record.storage_key = storage_key_result
            db_record.status = "ready"
            db_record.storage_url = storage_url
    ...
    def find_by_keys(self, keys: List[str]) -> List[EpubMetadata]:
        if not keys:
            return []
        with get_session() as session:
            result = session.execute(select(EpubMetadata).where(EpubMetadata.storage_key.in_(keys)))
            return list(result.scalars().all())

    def get_all(self) -> List[EpubMetadata]:
        return self.list_epubs()

    def download_buffer(self, storage_key: str):
        return self.storage.download_object(storage_key)

    def generate_presigned_url(self, storage_key: str) -> str:
        return self.storage.generate_presigned_url(storage_key)
    ...
    def _generate_and_upload(
        ...
        *,
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            upload_result = self.storage.upload_file(epub_path, storage_key)

            if hasattr(upload_result, "download_url") and hasattr(upload_result, "file_id"):
                final_storage_key = upload_result.file_id
                storage_url = upload_result.download_url
            else:
                final_storage_key = storage_key
                storage_url = upload_result

            return final_storage_key, storage_url, file_size
